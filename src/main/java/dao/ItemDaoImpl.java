...
public class ItemDaoImpl implements ItemDao {
	...
	public void create(final Item item) {
		this.template.execute(INSERT, new AbstractLobCreatingPreparedStatementCallback(lobHandler) {
			@Override
	...
	public void delete(Item item) {
		this.template.update(DELETE, item.getItemId());
	}
	...
	public List<Item> findByItemName(String itemName) {
		RowMapper<Item> mapper = new BeanPropertyRowMapper<Item>(Item.class);
		return this.template.query(SELECT_BY_ITEM_NAME, mapper, itemName + "%");
	}
	...
	public void udpate(final Item item) {
		this.template.execute(UPDATE, new AbstractLobCreatingPreparedStatementCallback(lobHandler) {
			@Override
			protected void setValues(PreparedStatement ps, LobCreator lobCreator) throws SQLException, DataAccessException {
				int index = 0;
				ps.setString(++index, item.getItemName());
